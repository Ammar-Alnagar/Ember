...
[Safetensors](https://github.com/huggingface/safetensors) is a fast and safe persistence format for deep learning models, and is required for tensor parallelism. Ember supports `safetensors` model loading under the hood. By default, given a repository with `safetensors` and `pytorch` weights, Ember will always load `safetensors`. If there's no `pytorch` weights, Ember will convert the weights to `safetensors` format.
