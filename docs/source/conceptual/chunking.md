...
Performance leap: Ember processes 3x more tokens, 13x faster than vLLM on long prompts. Zero config !
...
### 13x faster
On long prompts (200k+ tokens) conversation replies take 27.5s in vLLM, while it takes only 2s in Ember. How so ? We keep the initial conversation around, so when a new reply comes in, we can answer almost instantly. The overhead of the lookup is ~5us. Thanks @Daniël de Kok for the beast data structure.

### Zero config
That’s it. Remove all the flags your are using and you’re likely to get the best performance. By evaluating the hardware and model, Ember carefully selects automatic values to give best performance. In production, we don’t have any flags anymore in our deployments. We kept all existing flags around, they may come in handy in niche scenarios.
...
Ember: `text-generation-launcher --model-id $MODEL_ID --num-shard $N --port 8000` (or docker variant)
...
![benchmarks_v3](https://raw.githubusercontent.com/huggingface/ember/refs/heads/main/assets/v3_benchmarks.png)
...
|   |   |   |   |   |
|---|---|---|---|---|
|2nd run ||**Ember v3** (time in s)|**vLLM** (s)|**Amount of req**|
...
||||||
|1st run ||Ember (s)|vLLM (s)|Amount of req|
...
3.  **Multimodal models**: We are also investigating quite a lot other kind of models, like audio-to-audio, image/video generation, and other hybrids, where we see a lot of potential of applying the same principles we've applied in Ember to maximize performance.
